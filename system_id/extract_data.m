...
switch sim_type
    case 'HITL'
        ...
        if reload_data
            [file_name,parent_dir] = uigetfile([getenv('HOME'), '/Masters/Developer/MATLAB/Quad_Sim_Murray/system_id/HITL/*.csv'], '[extract_data.m] Choose HITL log DATA csv file (from logger.py)')
...
%                 y_data_noise = [vel.x]; % Data still noisy
        ...
        y_data_smooth = smoothdata(y_data_noise, 'loess', 80);
        u_data_smooth = smoothdata(u_data_noise, 'gaussian', 50); % Smooth u differently because of non-differentialable spikes
...
%         ylim([-4.4410    6.5628])
    ...
    case 'SITL'
        ...
        % Load data from csv into matrix (csv file created with payload_angle.py)
        if reload_data
            [file_name,parent_dir] = uigetfile([getenv('HOME'), '/Masters/Developer/MATLAB/Quad_Sim_Murray/system_id/SITL/*.csv', '[extract_data.m] Choose SITL log DATA csv file (from logger.py)'])
            data_path = strcat(parent_dir, file_name);
            data = readmatrix(data_path);
        end
...
if seperate_test_file
    ...
    if strcmp(sim_type, 'HITL') % Ignore test trian split for HITL ??
        % Test/Train split
